...
with DAG(
    ...
    dataset = "yellow_tripdata_{{ macros.ds_format(ds, '%Y-%m-%d', '%Y-%m') }}"

    download_dataset = BashOperator(
        task_id="download_dataset",
        bash_command=f"curl -fsS {dataset_url + dataset}.csv > {path_to_local_home}/plugins/{dataset}.csv"
    )
    ...
    convert_csv_to_parquet = PythonOperator(
        ...
        op_kwargs={
            "file_path": f"{path_to_local_home}/plugins/{dataset}.csv",
        },
    )
    ...
    upload_to_gcs = BashOperator(
        ...
        gsutil -m cp {path_to_local_home}/plugins/{dataset}.parquet gs://{BUCKET}",
